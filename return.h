...
public:
  explicit Return(Store *s, char code, int customerID,
...
private:
  // Return format: B CustomerID MediaType MovieType (movie sorting
  // attributes)
...
public:
  explicit ReturnFactory(Store *s);
  Command *makeCommand(const string &line, Store *s) const override;
};

string Return::genreName = "Return";
#endif